"""新規前処理6手法のLOSO-CV評価 (Round 3) - 軽量版

di-PLS, KMM, Subspace Alignment, Constituent EMSC, MMD Selection, JSMKPLS
"""
import sys
import numpy as np
import pandas as pd
...
from src.preprocessing.issue45_constituent_emsc import constituent_emsc
from src.preprocessing.issue46_mmd_selection import select_best_preprocessing_detailed
from src.preprocessing.issue47_jsmkpls import jsmkpls_transform
...
def main():
    ...
    # PCA次元削減 (di-PLS, KMM高速化用)
    pca50 = PCA(n_components=50).fit(X_train)
    X_pca = pca50.transform(X_train)
    ...
    for nc in [2, 4]:
        for lam in [0.1, 1.0, 10.0]:
            ...
            for train_idx, test_idx in folds:
                ...
                X_te = X_pca[test_idx]
                try:
                    preds = fit_predict_dipls(X_tr, y[train_idx], X_te,
            ...
            p(f"  diPLS(nc={nc},λ={lam}): RMSE={mean_r:.2f} ± {std_r:.2f}")
            results.append({"method": f"diPLS(nc={nc},λ={lam})", "rmse": mean_r, "rmse_std": std_r})

    # di-PLS + sqrt(y)
    for nc, lam in [(2, 1.0), (4, 1.0), (4, 0.1)]:
        fold_rmses = []
        for train_idx, test_idx in folds:
            X_tr = X_pca[train_idx]
            X_te = X_pca[test_idx]
            try:
                preds_sqrt = fit_predict_dipls(X_tr, np.sqrt(y[train_idx]), X_te,
                                              n_components=nc, dipls_lambda=lam)
                preds = preds_sqrt ** 2
                fold_rmses.append(rmse(y[test_idx], preds))
            except Exception:
        ...
        mean_r = np.mean(fold_rmses)
        std_r = np.std(fold_rmses)
        p(f"  diPLS(nc={nc},λ={lam})+sqrt(y): RMSE={mean_r:.2f} ± {std_r:.2f}")
        results.append({"method": f"diPLS(nc={nc},λ={lam})+sqrt(y)", "rmse": mean_r, "rmse_std": std_r})
    ...
    for nc in [2, 4]:
        for B in [10.0, 50.0]:
            ...
            for train_idx, test_idx in folds:
                ...
                X_te = X_pca[test_idx]
                try:
                    weights = compute_kmm_weights(X_tr, X_te, B=B)
                    preds = weighted_pls_predict(X_tr, y[train_idx], X_te, weights, n_components=nc)
                    fold_rmses.append(rmse(y[test_idx], preds))
                except Exception:
                    fold_rmses.append(999.0)
            mean_r = np.mean(fold_rmses)
            std_r = np.std(fold_rmses)
            p(f"  KMM(B={B})+PLS({nc}): RMSE={mean_r:.2f} ± {std_r:.2f}")
            results.append({"method": f"KMM(B={B})+PLS({nc})", "rmse": mean_r, "rmse_std": std_r})
    ...
    for nc in [4]:
        for B in [10.0]:
            ...
            for train_idx, test_idx in folds:
                ...
                # KMMはPCA空間で計算、PLSはraw空間で
                weights = compute_kmm_weights(X_pca[train_idx], X_pca[test_idx], B=B)
                preds = weighted_pls_predict(X_tr, y[train_idx], X_te, weights, n_components=nc)
                fold_rmses.append(rmse(y[test_idx], preds))
            mean_r = np.mean(fold_rmses)
            std_r = np.std(fold_rmses)
            p(f"  KMM_raw(B={B})+PLS({nc}): RMSE={mean_r:.2f} ± {std_r:.2f}")
            results.append({"method": f"KMM_raw(B={B})+PLS({nc})", "rmse": mean_r, "rmse_std": std_r})

    # ===== 3. Subspace Alignment =====
    p("\n=== Subspace Alignment + PLS ===")
    for d in [5, 10, 20]:
        fold_rmses = []
        for train_idx, test_idx in folds:
            X_tr, X_te = X_train[train_idx], X_train[test_idx]
            Z_tr, Z_te = subspace_align(X_tr, X_te, n_components=d)
            nc_pls = min(d, 4)
    ...
    # MMD-selected preprocessing でLOSO-CV
    best_preproc = preprocessings[best_pp["name"]]
    X_best = best_preproc(X_train)
    for nc in [2, 4]:
        fold_rmses = []
        for train_idx, test_idx in folds:
            pls = PLSRegression(n_components=nc)
    ...
    for n_comp in [5, 10]:
        ...
        for train_idx, test_idx in folds:
            ...
            X_te = X_pca[test_idx]
            try:
                Z_tr, Z_te = jsmkpls_transform(X_tr, X_te, n_components=n_comp,
                                                mu=1.0, lam=1.0, eta=1.0)
                nc_pls = min(n_comp, 4)
                pls = PLSRegression(n_components=nc_pls)
                pls.fit(Z_tr, y[train_idx])
                preds = pls.predict(Z_te).ravel()
    ...
    for train_idx, test_idx in folds:
        try:
            Z_tr, Z_te = jsmkpls_transform(X_pca[train_idx], X_pca[test_idx],
                                            n_components=10)
            pls = PLSRegression(n_components=4)
            pls.fit(Z_tr, np.sqrt(y[train_idx]))
            preds = pls.predict(Z_te).ravel() ** 2
