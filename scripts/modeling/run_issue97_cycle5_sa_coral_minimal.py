"""Issue #97: サイクル5 - SA + CORAL 最小版（8分以内完了を目標）

SA: pp=[raw, SNV] x nc=[3, 10, 20] x reg=[Ridge, PLS] x tf=[raw, sqrt] = 24
CORAL: pp=[raw, SNV] x pca=[10, 20, 50] x reg=[Ridge, PLS] x tf=[raw, sqrt] = 24
合計: 48モデル（EPOを除外して高速化）
"""
import sys
...
def preprocess(X_tr, X_te, pp_name):
    if pp_name == "SNV":
...
def main():
    ...
    print("=" * 70)
    print("Issue #97: サイクル5 - SA + CORAL 最小版")
    ...
    for pp_name in ["raw", "SNV"]:
        for sa_nc in [3, 10, 20]:
            for reg_type in ["Ridge", "PLS"]:
                for tf in ["raw", "sqrt"]:
                    cfg = {
                        "pp": pp_name, "sa_nc": sa_nc, "reg": reg_type, "tf": tf,
                    }
                    if reg_type == "PLS":
    ...
    coral_configs = []
    for pp_name in ["raw", "SNV"]:
        for pca_dim in [10, 20, 50]:
            for reg_type in ["Ridge", "PLS"]:
                for tf in ["raw", "sqrt"]:
                    cfg = {
                        "pp": pp_name, "pca_dim": pca_dim, "reg": reg_type, "tf": tf,
                    }
                    if reg_type == "PLS":
    ...
    def get_pp_cache(pp_name):
        if pp_name not in pp_cache:
            ...
            for f_idx, (train_idx, test_idx) in enumerate(folds):
                X_tr_pp, X_te_pp = preprocess(X_raw[train_idx], X_raw[test_idx], pp_name)
