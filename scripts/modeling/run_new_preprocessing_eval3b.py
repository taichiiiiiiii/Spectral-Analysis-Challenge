"""新規前処理6手法のLOSO-CV評価 (Round 3b) - KMM以降のみ

SA, EMSC, MMD Selection は軽量。KMM, JSMKPLSはPCA10次元で実行。
"""
import numpy as np
import pandas as pd
...
def main():
    ...
    # ===== 1. KMM (PCA20) =====
    p("=== KMM + PLS (PCA20空間で重み計算) ===")
    for nc in [2, 4]:
        fold_rmses = []
        for train_idx, test_idx in folds:
            weights = compute_kmm_weights(X_pca20[train_idx], X_pca20[test_idx], B=10.0)
            preds = weighted_pls_predict(X_train[train_idx], y[train_idx],
                                         X_train[test_idx], weights, n_components=nc)
            fold_rmses.append(rmse(y[test_idx], preds))
        mean_r = np.mean(fold_rmses)
        std_r = np.std(fold_rmses)
        p(f"  KMM+PLS({nc}): RMSE={mean_r:.2f} ± {std_r:.2f}")
        results.append({"method": f"KMM+PLS({nc})", "rmse": mean_r, "rmse_std": std_r})
    ...
    for train_idx, test_idx in folds:
        weights = compute_kmm_weights(X_pca20[train_idx], X_pca20[test_idx], B=10.0)
        P = compute_epo_projection(X_train[train_idx], groups[train_idx], n_components=1)
        X_tr_epo = apply_epo(X_train[train_idx], P)
        X_te_epo = apply_epo(X_train[test_idx], P)
        preds = weighted_pls_predict(X_tr_epo, y[train_idx], X_te_epo, weights, n_components=4)
        fold_rmses.append(rmse(y[test_idx], preds))
    mean_r = np.mean(fold_rmses)
    std_r = np.std(fold_rmses)
    p(f"  KMM+EPO(1)+PLS(4): RMSE={mean_r:.2f} ± {std_r:.2f}")
    results.append({"method": "KMM+EPO(1)+PLS(4)", "rmse": mean_r, "rmse_std": std_r})
    ...
    for train_idx, test_idx in folds:
        ...
        X_w = X_train[train_idx] * sqrt_w[:, None]
        y_w = np.sqrt(y[train_idx]) * sqrt_w
        pls = PLSRegression(n_components=4)
        pls.fit(X_w, y_w)
        preds = pls.predict(X_train[test_idx]).ravel() ** 2
        fold_rmses.append(rmse(y[test_idx], preds))
    mean_r = np.mean(fold_rmses)
    std_r = np.std(fold_rmses)
    p(f"  KMM+PLS(4)+sqrt(y): RMSE={mean_r:.2f} ± {std_r:.2f}")
    results.append({"method": "KMM+PLS(4)+sqrt(y)", "rmse": mean_r, "rmse_std": std_r})
